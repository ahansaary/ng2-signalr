...
export class SignalRConnection implements ISignalRConnection {
    ...
    let $promise = new Promise<ISignalRConnection>((resolve, reject) => {
      this._jConnection
        ...
        .done(() => {
          ...
          console.log(
            'Connection established, Transport: ' +
              this._jConnection.transport.name
          )
          resolve(this)
        })
        .fail((error: any) => {
  ...
  public invoke(method: string, ...parameters: any[]): Promise<any> {
    if (method == null) {
      throw new Error(
        "SignalRConnection: Failed to invoke. Argument 'method' can not be null"
      )
    }
  ...
  public listen<T>(listener: BroadcastEventListener<T | any[]>): void {
    if (listener == null) {
    ...
    let callback: CallbackFn = (...args: any[]) => {
      this._zone.run(() => {
        ...
        this.log(
          'SignalRConnection.proxy.on invoked. Calling listener next() ...'
        )
    ...
    this.log(
      `SignalRConnection: Starting to listen to server event with name ${
        listener.event
      }`
    )
  ...
  public stopListening<T>(listener: BroadcastEventListener<T>): void {
    ...
    this.log(
      `SignalRConnection: Stopping listening to server event with name ${
        listener.event
      }`
    )
  ...
  private convertTransports(
    transports: ConnectionTransport | ConnectionTransport[]
  ): any {
  ...
  private onBroadcastEventReceived<T>(
    listener: BroadcastEventListener<T>,
    ...args: any[]
  ) {
